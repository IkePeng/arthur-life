(function () {
  function pitchProfile(config,pitch,stats){
    ...
    let hit=.49-quality*.00145,k=.075+quality*.0018,ballAdjustment=0;
    const profile=pitchMap[pitch]?.profile;
    if(profile){hit+=profile.hitAdjustment;k+=profile.strikeoutAdjustment;ballAdjustment=profile.ballAdjustment||0;}
    ...
    return {quality,hit:Math.max(.21,Math.min(.56,hit)),k:Math.max(.07,Math.min(.44,k)),ballAdjustment,breaker};
  }
  ...
  function liveRisk(config,pitch,batter,match,stats){
    ...
    return {hit,ball:Math.max(balance.ballChance?.min??.05,Math.min(balance.ballChance?.max??.85,baseBallChance+x.ballAdjustment+fatigue+healthPenalty+protectZone)),miss:x.k-(batter.contact||0)*.35,note:situation};
  }
