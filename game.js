...
function startGame(customState) {
  ...
  state.position="投手"; state.pitches=state.pitches||["四縫線直球"]; state.bird=birdRoster[state.bird]?state.bird:"18"; state.origin=state.origin||state.bird; state.special=state.special||[]; state.pitchChoiceLevels=state.pitchChoiceLevels||[];state.pendingPitchUnlocks=state.pendingPitchUnlocks||[];state.fourSeamStreak=state.fourSeamStreak||0;state.sinkerStreak=state.sinkerStreak||0;state.sliderPitchCount=state.sliderPitchCount||0; state.stats.luck=state.stats.luck??35; state.usedLifeEvents=state.usedLifeEvents||[]; state.relationship=state.relationship||"single"; state.chapterRounds=state.chapterRounds||rollChapterRounds(state.stats.luck);state.stageYear=state.stageYear??0;state.age=state.age??(stageAges[state.chapter]?.[state.stageYear]||chapters[state.chapter]?.age||13);state.lastAgingAge=state.lastAgingAge??state.age;
