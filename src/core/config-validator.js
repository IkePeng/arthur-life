(function () {
  ...
  function validateConfig(c){
    ...
    if(c.batterTrashTalk){assert(Number.isFinite(c.batterTrashTalk.chance)&&c.batterTrashTalk.chance>=0&&c.batterTrashTalk.chance<=1,"batterTrashTalk.chance 必須介於 0～1",errors);["control","strike","twoStrikes","foul","hit","general"].forEach(k=>assert(Array.isArray(c.batterTrashTalk[k])&&c.batterTrashTalk[k].length,`batterTrashTalk.${k}：至少需要一句對白`,errors));}
